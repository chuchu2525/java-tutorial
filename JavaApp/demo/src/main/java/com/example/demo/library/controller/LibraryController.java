...
public class LibraryController {
    ...
    @PostMapping("/loans")
    public ResponseEntity<ActionResponse> borrowBook(@RequestBody LoanRequest request) {
        ActionResponse response = libraryService.borrowBook(request.getMemberId(), request.getBookId());
        return buildResponse(response);
    }

    @PostMapping("/returns")
    public ResponseEntity<ActionResponse> returnBook(@RequestBody ReturnRequest request) {
